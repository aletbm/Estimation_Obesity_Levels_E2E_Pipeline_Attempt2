...
# Cloud Run service
resource "google_cloud_run_service" "obesity-level-api" {
  name     = "obesity-level-api"
  location = var.region

  template {
    spec {
      containers {
        image = "gcr.io/${var.project}/obesity-level-api"
        ports {
          container_port = 8080
        }
      }
...
resource "google_cloud_run_service_iam_member" "noauth" {
  service  = google_cloud_run_service.obesity-level-api.name
  location = var.region
